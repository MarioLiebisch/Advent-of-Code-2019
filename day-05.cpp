...
void run (std::vector<value> memory) {
    ...
    // input
    codes[3]  = [&]() { std::cin >> read(ip + 1, mode1); ip += 2; };
    // output
    codes[4]  = [&]() { std::cout << read(ip + 1, mode1) << "\n"; ip += 2; };
    // jump if true
...
int main(int argc, char **argv) {
    /* for (size_t noun = 0; noun < 100; ++noun) {
        for (size_t verb = 0; verb < 100; ++verb) {
            ...
            if (result == 19690720) {
                std::cout << "Solution: " << (noun * 100 + verb) << std::endl;
                return 0;
            }
        }
    } */
